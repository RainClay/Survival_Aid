...
/* JADX INFO: loaded from: carpet-survival-aid-mc1.21.10-1.0.1.jar:com/survivalaid/SurvivalAidRuntime.class */
public final class SurvivalAidRuntime {
    ...
    private static void handleVoidPlayerRescue(MinecraftServer server, class_3222 player) {
        if (!VoidPlayerRescueRule.survivalAidVoidPlayerRescue || player.method_68878() || player.method_7325() || player.method_29504() || player.method_23318() > VoidPlayerRescueYRule.survivalAidVoidPlayerRescueY) {
            return;
        }
        ...
        class_1671 firework = new class_1671(player.method_51469(), rocketForUse, player);
        player.method_51469().method_8649(firework);
    ...
    private static void handleAutoEatFood(MinecraftServer server, class_3222 player) {
        if (!AutoEatFoodRule.survivalAidAutoEatFood || player.method_68878() || player.method_7325()) {
            debugAutoEat(server, player, "跳过：规则=" + AutoEatFoodRule.survivalAidAutoEatFood + "，创造=" + player.method_68878() + "，旁观=" + player.method_7325());
            return;
        }
